...
#Loading metadata
annotation.data <- read.delim(file = "./data/processed/metadata/metadata.tsv", sep = "\t", row.names = "sample.new")
...
k.wss.genes <- fviz_nbclust(mat_scaled[DEG.list,], kmeans, 
                            method = "wss", 
                            nboot = 100)
pdf(file = "./output/plots/Optimal_k_clusters_genes_gap.pdf", height = 4, width = 4)
...
k.wss.samples <- fviz_nbclust(t(mat_scaled[DEG.list,]), kmeans, 
                              method = "wss", 
                              nboot = 100)
pdf(file = "./output/plots/Optimal_k_clusters_samples_gap.pdf", height = 4, width = 4)
...
column.ha <- HeatmapAnnotation(df = annotation.data[, c("condition", "marsh.2")],
...
#First adjusting rows, having first groups genes very strong for CeD [4] > moderate [3] > small group downregulated [1] > big group downregulated [2] in CeD
...
clu_df_reordered <- gsub("Cluster_1", "Cluster_c", clu_df_reordered)
clu_df_reordered <- gsub("Cluster_2", "Cluster_d", clu_df_reordered)
...
column.ha.2 = HeatmapAnnotation(df = annotation.data[, c("condition", "marsh.2")],
                                APOA4_KI67 = anno_barplot(as.matrix(extra.metadata[,17]), 
                                                          height = unit(2, "cm"), 
                                                          gp = gpar(fill = col_fun(extra.metadata[,17])),
                                                          border = F),
...
extended_metadata <- cbind(extended_metadata)
...
#Saving Heatmap
pdf(file = "./output/plots/heatmap_DEG.pdf", width=12, height=8,  family = "ArialMT")
draw(ht_list2)
dev.off()
